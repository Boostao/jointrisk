...
#' @param prefix Prefix added to default input names. Default to "".
#' @param rvextbet of the input column corresponding to concrete siding percentage (9045). Default to RVEXTBET.
#' @param rvextbri of the input column corresponding to masonry siding percentage (9408). Default to RVEXTBRI.
#' @param murstruc of the input column corresponding to wall structure (959). Default to MURSTRUC.
#' @param plancher of the input column corresponding to flooring (9406). Default to PLANCHER.
#' @param toitstru of the input column corresponding to roof structure (971). Default to TOITSTRU.
#' @param resaufeu of the input column corresponding to fire resistant yes/no (14491). Default to RESAUFEU.
...
append_typecons <- function(dt,
  ...
  required <- c(rvextbet, rvextbri, plancher, toitstru, resaufeu)
  if (!all(required %chin% names(dt))) {
    notin <- required[!required %chin% names(dt)]
    stop(paste("Required fields", paste(notin, collapse = ", "), "not found in dt."))
  }

  data.table::set(dt,
                  j = "REVETEME",
                  value = c("N", "O")[1L + as.integer(rowSums(dt[,list(get(rvextbet), get(rvextbri))], na.rm = TRUE) > 59L)])

  dt[compo$COTYCONS,
     on = c(paste0(murstruc,"==MURSTRUC"),
            paste0(plancher,"==PLANCHER"),
            paste0(toitstru,"==TOITSTRU"),
            "REVETEME"),
     `:=`(TYPECONS = as.integer(TYPECONS))]

  dt[compo$COTYCON2,
     on = paste0(resaufeu,"==RESAUFEU"),
...
#' @param dt A data.table containing required fields.
#' @param output Name of the output column, created or modified. Default to RISKRADIUS.
#' @param prefix Prefix added to default input names. Default to "".
#' @param risasgrb Name of the input column corresponding to insured risk property group (14218). Default to RISASGRB.
#' @param comautba Name of the input column corresponding to property linked to other (14367). Default to COMAUTBA.
#' @param affectat Name of the input column corresponding to activity code (140). Default to AFFECTAT.
#' @param umessup2 Name of the input column corresponding to mesuring units (1092). Default to UMESSUP2.
#' @param typecons Name of the input column corresponding to construction type (10762). Default to TYPECONS.
#' @param tycons2  Name of the input column corresponding to construction type 2 (14492). Default to TYCONS2.
#' @param princfus Name of the input column corresponding to fire underwriters survey fire protection zone (14219). Default to PRINCFUS.
#' @param superrez Name of the input column corresponding to ground floor area (1083). Default to SUPERREZ.
...
calculate_radius <- function(dt,
                             ...
                             typecons = paste0(prefix,"TYPECONS"),
                             tycons2  = paste0(prefix,"TYCONS2"),
                             princfus = paste0(prefix,"PRINCFUS"),
                             superrez = paste0(prefix,"SUPERREZ")) {
  required <- c(risasgrb, comautba, affectat, umessup2, typecons, tycons2, princfus, superrez)
  if (!all(required %chin% names(dt))) {
    notin <- required[!required %chin% names(dt)]
    stop(paste("Required fields", paste(notin, collapse = ", "), "not found in dt."))
  }
...
create_polygons <- function(dt) {
 ...
 polygons <- dt[!is.na(LATITCOM) & !is.na(LONGICOM) & PREGEOCO %chin% c("POINTADDRESS","STREETADDRESS","SUBADDRESS"),
              list(INTE_NO, POAS_NO, PRCH_NO, PRCH_ID, MTTOTRAS, LONGICOM, LATITCOM, RISKRADIUS, RISASGRB)]

 polygons <- sf::st_as_sf(polygons[, c(LONG, LAT) := list(LONGICOM, LATITCOM)],
